# Importing Dataset
Titanic <- read.csv("F:\\Data Science with R\\Session5-20220505T073814Z-001\\Session5\\03. Titanic visualization\\titanic.csv", stringsAsFactors = FALSE)
...
#The graph is partitioned in multiple panels by levels of pclass:
...
#theme_bw() is a simple black and white theme.
...
#variable. It uses a kernel density estimate to show the 
#probability density function of the variable.
#It is a smoothed version of the histogram.
# Plot for age group survival distribution
#theme_bw() is a simple black and white theme.
ggplot(Titanic, aes(x = age, fill = survived)) + 
  theme_bw() + geom_density(alpha=0.5) +
  labs(y = "passenger count", x = "Age", 
       title = "Titanic Age distribution")
